...
def clip_weights(w, min_val=1e-6):
    w = ensure_numpy(w)
    w = np.nan_to_num(w, nan=min_val, posinf=min_val, neginf=min_val)
    w = np.where(w < min_val, min_val, w)
...
def ensure_feature_mask(X, mask):
    ...
    if not mask.any():
        raise ValueError(
            "Feature mask removed ALL features. At least one feature must remain."
        )
